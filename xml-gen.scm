; ~/~ begin <<docs/svg-gen.md#xml-gen.scm>>[init]
(import (rnrs)
        (rnrs eval)
        (ice-9 match)
        (ice-9 format))

; ~/~ begin <<docs/svg-gen.md#main>>[init]
(define (read-all)
  (do ((x (read) (read))
       (r '() (cons x r)))
      ((eof-object? x) (reverse r))))
; ~/~ end
; ~/~ begin <<docs/svg-gen.md#main>>[1]
